...
type ContextCredentialsFactory interface {
	...
	// FetchIAMAccessToken returns a config from iam which is derived from a trusted profile
	FetchIAMAccessToken(logger *zap.Logger) (provider.ContextCredentials, error)
}
